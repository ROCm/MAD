...
def test_a_stage_in_two_categories_is_summed_not_overwritten():
    """A stage appearing in two categories has both summed into its row, not overwritten."""
...
def test_an_arms_share_does_not_move_with_what_the_other_arm_classified():
    """Each arm's share is computed over its own categories, not the union of both arms'."""
...
def test_the_summary_can_only_name_variants_the_table_shows():
    """The variant summary names only the variant the first-match-wins table shows."""
...
def test_a_split_arm_does_not_report_the_other_arm_as_split():
    """A per-node split is marked only on the arm that has it, not in both columns."""
...
def test_the_markdown_and_the_csv_list_the_same_settings():
    """The markdown and the CSV list the same settings, both taken from `build_config_diff`."""
...
def test_an_arm_whose_nodes_disagree_has_not_reported_no_configuration():
    """An arm whose nodes disagree is reported as disagreeing, not as reporting no
    configuration."""
...
def test_a_kernel_that_ran_is_not_reported_as_zero_calls():
    """A kernel averaging under one call per trace keeps a fractional count, not zero."""
...
class TestLoadBenchmark:
    ...
    def test_a_repeated_measurement_keeps_the_first_and_says_so(self, tmp_path: Path, capsys):
        """A measurement reported twice keeps the first value and warns about the second."""
        (tmp_path / "perf_a.csv").write_text(
            self.HEADER + "2p2d_isl1024_osl1024_con64,226.3,mean_itl_ms\n")
...
def test_out_and_out_dir_cannot_both_be_given(capsys):
    """`--out` and `--out-dir` together are rejected rather than one silently winning."""
    import pytest
...
def test_an_engine_that_declares_no_benchmark_layout_loses_only_those_sections():
    """An engine declaring no benchmark layout gets no benchmark rows, rather than another
    engine's column names."""
    from dataclasses import replace
...
class TestOutputDirectoryOwnership:
    ...
    def test_a_csv_this_command_did_not_write_is_left_alone(self, tmp_path, monkeypatch):
        """A CSV this command did not write is left in place when the output directory is
        swept."""
        out = tmp_path / "out"
        out.mkdir()
...
def test_a_variant_below_the_display_cut_still_triggers_the_mismatch_warning():
    """A variant carried only by a kernel below the display cut still triggers the mismatch
    warning."""
...
def test_batch_sensitivity_is_not_reported_as_a_broken_estimate():
    """Batch sensitivity is reported as batch-sensitive, not as an unreliable estimate."""
    stats = summarise_steps([StepRecord(batch=64, rate=280.0)] * 8)
...
def test_an_uncaptured_arm_is_named_before_any_classification_claim():
    """An arm with no trace is named as uncaptured, and classification claims name only the
    captured arm."""
    from collprof.core.compare import section_a2a
...
def test_one_silent_node_does_not_erase_what_the_others_stated():
    """Labelling the arm `not stated` because a second node said nothing hid the graph mode the
    first node did report -- a per-step confound this table exists to surface."""
    from collprof.core.compare import build_steps

    off = summarise_steps([StepRecord(batch=64, rate=280.0, graphed=False)] * 8)
    silent = summarise_steps([StepRecord(batch=64, rate=280.0)] * 8)
    arm = Arm(name="A", run_dir=Path("/tmp"), config={}, trace=None, points={},
              steps={"NODE0": off, "NODE1": silent}, pooled=off)
    other = Arm(name="B", run_dir=Path("/tmp"), config={}, trace=None, points={},
                steps={"NODE0": off}, pooled=off)

    rows = {r[0]: r[-1] for r in build_steps(arm, other)[1]}

    assert rows["A"] == "off, 1 node(s) not stated"
    assert rows["B"] == "off", "an arm where every node spoke is unchanged"
