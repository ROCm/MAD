...
# Both sections exist to surface configuration confounds between two compared arms.
...
def test_comparing_a_graph_free_arm_against_a_graphed_one_flags_the_difference(
    ...
    """Arms differing in graph capture are reported as a configuration difference, not as
    comparable."""
...
def test_a_silent_node_does_not_become_a_claim_that_every_step_was_ungraphed(
        sglang_run: Path, tmp_path: Path):
    """A node that never stated its graph state is counted as unknown, not as ungraphed."""
...
def test_a_run_without_collectives_says_so_not_zeros(sglang_run: Path, tmp_path: Path):
    """A run with no COLL records reports the channel as unavailable and writes no traffic
    CSVs."""
    for node in (2, 3):
...
def test_the_file_list_names_only_files_that_exist(sglang_run: Path, tmp_path: Path):
    """The report's file list names only CSVs that were actually written."""
    for node in (2, 3):
        write(sglang_run / f"decode_NODE{node}.log",
              [server_args_line(), decode_batch_line(batch=16, rate=80.0)])
...
def test_a_phase_without_collectives_does_not_report_zero_traffic(tmp_path: Path):
    """The summary of a phase with no COLL rows says traffic is unavailable rather than zero."""
...
def test_a_phase_with_only_metrics_still_gets_a_report(tmp_path: Path):
    """`parse_run` records metrics and the summary renders them, but the CLI's emptiness check did
    not count them, so a metrics-only phase was skipped and produced no report at all."""
    run = tmp_path / "job"
    (run / "node_0").mkdir(parents=True)
    (run / "node_0" / "stdout.out").write_text(
        "[INFO] [main] Executing: bash runner/primus-cli-direct.sh -- train pretrain "
        "--config examples/megatron/configs/MI355X/llama3.1_70B-BF16-pretrain.yaml\n"
        " iteration 10/500 | elapsed time per iteration (ms): 250.0 |\n"
        " iteration 11/500 | elapsed time per iteration (ms): 251.0 |\n")

    phase = parse_run(run, engines.get("primus"))["BF16"]
    assert not (phase.sizes or phase.steps or phase.config), "metrics are the only channel"
    assert phase.metrics

    # Through the CLI, which is where the skip decision lives.
    reports = build(run, tmp_path / "out", ["--engine", "primus"])

    assert "BF16" in reports, "a phase the report can render is not nothing to report"
