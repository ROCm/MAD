...
Classification is by event name, so unknown names must report nothing and say which names were
busiest.
"""
...
class TestTraceExtraction:
    ...
    def test_an_event_without_a_category_does_not_land_in_the_previous_one(self, tmp_path: Path):
        """An event without `cat` is not charged to the previous event's category."""
        path = trace_with(named_trace_event("mori_dispatch", dur=250.0),
...
class TestKernelVariants:
    """The variant column names which implementation ran, on both axes: latency vs throughput,
    and intranode vs internode."""
    ...
    def test_both_axes_at_once_keep_both(self):
        """A kernel stating both axes keeps both, rather than matching one and dropping the
        other."""
...
def test_an_a2a_kernel_with_a_spliced_duration_keeps_its_call(tmp_path: Path):
    """An a2a kernel with an unreadable duration is still counted as a call."""
...
def test_the_kernel_table_is_built_from_kernels_only(tmp_path: Path):
    """`a2a_names`, which feeds the kernel table and variant summary, holds kernels only and not
    the host ops that launch them."""
    write(tmp_path / "1000.0-TP-0.trace.json",
          ['{"traceEvents": [']
...
def test_a_name_on_its_own_line_still_names_the_kernel(tmp_path: Path):
    """An event whose `name` sits on a different line from its `cat` is still classified."""
    split_event = (
        '    {\n'
        '      "ph": "X", "cat": "kernel",\n'
