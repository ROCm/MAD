...
disagree. The engine is detected from the left run and the right must match.
"""
...
#: What the previous run wrote into `--out-dir`, so this one knows what is its to delete: the
#: directory belongs to the caller and may hold files this command never wrote.
...
def _counters_of(counters: dict, phase: str, spec) -> dict:
    ...
    Matched on the label, not the phase's node set: a node that logged no collectives still has an
    adapter carrying this run's traffic.
    """
...
def parse_side(run_dir: Path, spec, cache: ParseCache, want_traces: bool) -> tuple:
    ...
    ``empty_dirs`` are capture directories that held no trace file: dropped, but not silently, or
    an arm covering one replica reads as comparable to an arm covering two.
    """
...
def main(argv: list | None = None) -> None:
    ...
    # headers), so two arms sharing a label overwrite each other.
    ...
    if args.out_dir:
        ...
        # the stale-CSV sweep below, delete the perf CSV it parsed.
        ...
        # Nothing is overwritten that this command did not write: the manifest decides ownership
        # for writes as it does for deletions.
        ...
        # Stale CSVs from an earlier run would come back as sheets that do not match the rest of
        # the workbook. What may be deleted is decided by the manifest, not by the name.
        ...
        # this directory may hold unrelated ones.
        book = write_workbook(args.out_dir, text.splitlines(), filename="comparison.xlsx",
                              only={f"{name}.csv" for name in produced})
        # This run's artifacts, not the directory listing, which may hold unrelated files.
