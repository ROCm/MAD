...
prose in :class:`~collprof.core.spec.ReportNotes` and is inserted, never asserted here.
"""
...
class CsvSink:
    ...
    def drop_stale(self, only: set | None = None) -> None:
        ...
        ``only`` protects input files when a comparison writes into a run directory it also reads.
        """
...
def write_tables(view: PhaseView, sink: CsvSink, ctx: ReportContext) -> dict:
    ...
    step_rows = []
    # A withheld channel is withheld everywhere, including this CSV and the workbook sheet.
    ...
    for (stage, cat), (calls, dur_us) in sorted((trace.get("a2a") or {}).items()):
        ...
        # No absolute duration: trace durations fail a cross-check against rocprofv3. Shares only.
...
def section_header(view: PhaseView, ctx: ReportContext) -> list:
    ...
    lines = [
        ...
        # From every channel: `Phase.nodes` fills only when a collective record parses, so a
        # tuned run would otherwise report zero contributing nodes.
        f"Nodes contributing log data: {len(_contributing(phase, ctx))} "
        f"({', '.join(sorted(_contributing(phase, ctx))) or 'none'})  ",
...
def counter_table(view, ctx: ReportContext) -> list:
    ...
    A sampled node can yield no rows, so the file list must follow the rows rather than the
...
def section_summary(view: PhaseView, ctx: ReportContext) -> list:
    ...
    lines = ["", "## Summary", ""]
    # Only when collectives parsed: a run with configuration and counters but no COLL row would
    # otherwise print zeros for a measurement that was never taken.
    if phase.sizes:
        lines += [
    ...
    if not ctx.rocprof:
        # The engine's own step time is a duration without a profiler, so the notice has to name
        # the gap as a *device* duration.
...
def section_data_quality(view: PhaseView, ctx: ReportContext) -> list:
    ...
    Bounds are calibrated to a run's scale, so a larger topology or model can have real records
    rejected; hitting one is called out with the flag that raises it.
    """
...
def section_run_config(view: PhaseView, ctx: ReportContext) -> list:
    ...
    if not config:
        # A failed comparison must still be stated; a missing section reads as one never asked
        # for.
    ...
    # `host`, `port`, `node_rank` and the seeds differ per node by design, so listing them would
    # fire this warning on every healthy multi-node run.
...
def section_measurement_check(agreed: dict, ctx: ReportContext,
    ...
    A report claiming a measurement configuration while showing a tuned run's traffic is worse
    than one that claims nothing.
    """
    # `disagreed` too: a key set differently on one node is a failure there, and checking only
    # agreed keys would miss it.
    contradictions = []
...
def section_diff_unavailable(ctx: ReportContext, heading: str) -> list:
    ...
    The heading is the caller's, since the two paths that reach this sit at different depths.
    """
...
def section_config_diff(ctx: ReportContext) -> list:
    ...
    for which, split in sorted((ctx.config_diff_split or {}).items()):
        # Every non-noise key: a split key is absent from the diff and from the agreed CSV rows,
        # so this is the last chance to mention it.
        keys = sorted(k for k in split if k not in ctx.spec.run_config.noise)
        if keys:
            moving = [k for k in keys if k in perf]
            # The diff is blind to every split key, so "no setting differs" over one is not a
            # finding.
            split_seen = True
    ...
    if not diff:
        # A split can be why the diff is empty, so do not declare the runs comparable then.
    ...
    for setting in diff:
        # A setting one side never reported is absent, which is not the same as unset.
...
def section_steps(view: PhaseView, ctx: ReportContext) -> list:
    ...
    Computed by the server rather than an instrument, so it is comparable between a profiled and
    an unprofiled run.
    """
    ...
    # `None` means the node did not state its graph mode; treating it as disagreement would make
    # a definite claim out of the unknown.
    graph_states = {st.graphed for st in per_node.values()} - {None}
    # `graphed is None` also covers a node whose own intervals disagreed, which the set above
    # hides; ask the nodes directly.
    mixed_within = any(st.graphs_mixed for st in per_node.values())
    if False in graph_states or mixed_within:
        # Three states, three claims: `graph_states` drops `None`, so `{False, None}` reads as
        # `{False}`. Ask the per-node summaries directly.
...
def _contributing(phase, ctx: ReportContext) -> set:
    """Every node this phase has data from, by any channel.

    A node that logged no collective still contributed if it reported a configuration, a step or
    counters.
    """
    return (set(phase.nodes) | set(phase.config_nodes) | set(phase.steps)
            | set(phase_counters(phase, ctx)))
...
def phase_counters(phase, ctx: ReportContext) -> dict:
    ...
    alone. By the engine's declared rule, not by which nodes reached the logs.
    """
...
def section_counters(view: PhaseView, ctx: ReportContext) -> list:
    ...
    It reports verbs, not causality, and counts every user of the NIC, so it bounds the exchange
    ...
    # Only the nodes with a window: a zero row for a node sampled once is partial collection
    # presenting itself as a measurement. Those are named below the table.
    ...
    # Present, not non-zero: a measured zero is a fact, and dropping the column would say instead
    # that the counter was never there.
    shown = [k for k in kind_order(ctx.spec.counters, present) if k in present]
    if not shown:
        # Every counter that moved also wrapped, so `parse_counters` dropped them all. The rows
        # exist and have no columns; a table of empty cells would read as a run that sent nothing.
        lines.append("The nodes sampled a window and no counter survived it -- every one that "
                     "moved also wrapped or was reset, which is named below.")
        return lines + _counter_notes(view, ctx, series, omitted)
    ...
    # The asymmetry is the point: reads and atomics in quantity are evidence of a protocol that
    # waits; their absence is not evidence of one that does not.
...
def _counter_notes(view: PhaseView, ctx: ReportContext, series: dict, omitted: list) -> list:
    """What the table leaves out, stated whether or not a table was rendered."""
    lines: list = []
    if omitted:
    ...
    # A node with no counter file at all is invisible in `series`, so the check above cannot see
    # it. The other channels are a lower bound on the phase's nodes, not the truth.
    unsampled = sorted(_contributing(view.phase, ctx) - set(series))
    if unsampled:
        lines += ["", f"**{len(unsampled)} node(s) of this phase have no counter file at all** "
                      f"({', '.join(unsampled)}). They appear in the other channels, so the "
                      "sampler did not run there or its output did not arrive; what their "
                      "adapters carried is not in this table and is not zero."]

    broke = sorted(node for node, s in series.items() if s.sampler_failed)
    if broke:
        lines += ["", f"**The sampler reported it could not finish on {len(broke)} node(s)** "
                      f"({', '.join(broke)}). What it wrote is real; where its window ends is "
                      "not this run's end, so read those rows as a floor and not as a total."]
...
def section_a2a(view: PhaseView, ctx: ReportContext) -> list:
    ...
    Classification is by event name, so this is a discovery aid: it says what matched, and says
    nothing rather than zero when nothing did.
    """
    ...
    # The collective section returns early on a capture with no `record_param_comms` events,
    # which is exactly the capture this section describes.
...
def trace_quality_notes(trace: dict | None) -> list:
    ...
    Shared: the collective section returns early on captures with no ``record_param_comms``
    events, which would leave the a2a shares with nothing said about unreadable traces.
    """
    if not trace:
        return []
    lines: list = []
...
def section_files(view: PhaseView, ctx: ReportContext, tables: dict) -> list:
    ...
    Writing is conditional, so the list must be too: naming a file that is not there misleads.
    """
    lines = ["", "## Files", ""]
    if view.phase.sizes:
...
def emit_phase(phase: Phase, out_dir: Path, ctx: ReportContext) -> Path:
    ...
    # Only when there are collectives: an empty section reads as a run that issued none.
    if phase.sizes:
