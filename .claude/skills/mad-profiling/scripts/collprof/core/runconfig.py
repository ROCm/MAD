...
Attention backend, quantisation and profiling flags explain more than the communication numbers
do, so `diff_configs` puts them in one table, performance-relevant settings first. A difference is
...
def merge_nodes(config: dict, expected: set | None = None) -> tuple[dict, dict]:
    ...
    nodes whose startup line parsed, so a node that differs vanishes instead of dissenting.
