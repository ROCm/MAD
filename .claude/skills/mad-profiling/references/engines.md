...
produces the report it produced before they existed. Fill them in for anything that serves —
`run_config` is what makes two runs comparable or not, `steps` is the only duration channel a
serving run has, `a2a` the only one that names traffic a backend carries itself, and `benchmark`
is where throughput and latency come from at all, since a profiled run's own log carries neither.

`benchmark` exists because `core/` otherwise hardcodes one harness's schema — the key shape
`isl…_osl…_con…`, the metric names `mean_itl_ms` and `mean_ttft_ms`, the glob `perf_*.csv` — and a
second serving engine would have to edit `core/` to be read at all. Naming them here keeps the
end-to-end split arithmetic over whatever the harness called its columns.
...
6a. **Say where the engine states its own configuration, and its step time.** Both are one
   ...
   - `steps`: a line carrying enough to derive a step time. For a server that logs its running
     ...
     `graphed` group records whether a captured graph was replayed over those steps — the
     configuration difference most likely to explain a per-step gap, and in band rather than
     inferred from startup.

6b. **Declare `a2a` if a backend carries collectives outside RCCL.** An expert-parallel MoE
   backend does: MoRI over IBGDA, DeepEP over rocSHMEM, reaching neither an RCCL log nor a
   `record_param_comms` event. Patterns are matched against trace event names, first match wins, so
   order them from specific to general. Errors are cheap and visible: the report lists the busiest
   unclassified device events so the patterns can be corrected against a real trace.
...
7. **Write the notes.** Ask: what would a reader conclude from these numbers that is
   ...
   The same for the last three: the core reports that graph replay was off, and only the engine
   knows whether that is a deliberate measurement setting or a fault.
