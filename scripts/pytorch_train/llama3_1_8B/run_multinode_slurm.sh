...
srun bash -c '
    docker pull docker.io/rocm/pytorch-training:v25.7
    ...
    docker run -d --network host --device /dev/dri --device /dev/kfd --device /dev/infiniband \
      ...
      -v $HOME:$HOME -v  $HOME/.ssh:/root/.ssh --shm-size 128G --name training_env docker.io/rocm/pytorch-training:v25.7 tail -f /dev/null
'
