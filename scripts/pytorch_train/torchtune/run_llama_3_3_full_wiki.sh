...
CONFIG="${CONFIG:-./llama_3_3_70b_full_finetune_recipe.yaml}"
...
if [ ! -d $MODEL_DIR ]; then
    ...
    echo "Download with: "
    echo "  huggingface-cli download meta-llama/Llama-3.3-70B-Instruct --local-dir $MODEL_DIR --exclude 'original/*.pth'"
    exit 1
fi
