...
if [ ! -d "$MODEL_DIR" ]; then
    ...
    hf download "$HF_MODEL_NAME" --local-dir "$MODEL_DIR" --exclude "original/*.pth" || {
...
# === Always trim tensor parallel and data parallel keys for llama4 ===
if [[ "${MODEL_FAMILY,,}" == "llama4" ]]; then
  echo "Trimming tensor/data parallel config for llama4: $CONFIG_FILE"
  TMP_FILE=$(mktemp)
  in_tensor_plan="false"
