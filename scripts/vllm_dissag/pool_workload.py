...
The wrappers do NOT do that by default -- their default is one warm-up pass plus one
measured pass over a SINGLE trace, and they report the achieved mean and its deviation
rather than averaging the error away, because ten seeds cost ten times the wall clock.
Run them with SLO_ITERS=10 RESAMPLE_PER_ITER=1 when the target must be pinned. Given a
single trace this script still runs and is still useful -- it recomputes percentiles
from the raw lengths of that one sample -- but the figure it prints is that sample's,
carrying the full 4.0%/9.5% standard error above.
