...
| `connectors/offloading.inc.sh` | tiered prefix caching overlay: `kv_offload_wrap()` composes the disagg connector + the CPU-tier backend (`OFFLOAD_BACKEND=native`→`OffloadingConnector` \| `lmcache`→`LMCacheConnectorV1`) via `MultiConnector` when `KV_OFFLOAD=cpu` (no-op when `none`); `kv_offload_setup_env()` exports LMCache's env |
