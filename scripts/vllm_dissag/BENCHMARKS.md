...
| **avg 80K / 256K ctx** | Do we meet the TTFT/TPOT SLO under the customer's smaller workload? | `benchmark_avg_80K_ten.sh` |
| **avg 200K / 1M ctx** | Same, under the larger one. | `benchmark_avg_200K_ten.sh` |
...
```bash
MODEL_PATH=/models/GLM-5.2-FP8 BENCHMARK_PORT=30000 ./benchmark_avg_80K_ten.sh
MODEL_PATH=/models/GLM-5.2-FP8 BENCHMARK_PORT=30000 ./benchmark_avg_200K_ten.sh
```
...
Ten runs at **distinct** seeds pool to `n×10`, and the error falls as `1/sqrt(10)`: to
**1.3%** and **3.0%**. Reaching 1.3% inside a *single* run would need n=1,014 requests at
~80K tokens each — more than `MAX_PROMPTS` and far more wall clock than ten smaller runs,
for the same statistical answer.

This is what `RESAMPLE_PER_ITER=1` does, and it is the whole reason these two wrappers exist:
...
- `RESAMPLE_PER_ITER=0` *(default)* — one sample reused across iterations. Iterations measure
  **server variance on a fixed workload**. Use when A/B testing a code change.
...
| `SLO_ITERS` | `10` in the wrappers | iterations |
...
Each wrapper ends by pooling its samples and printing the distribution **actually served**:

```
Per-run realised means (this spread IS the sampling error, not a bug):
  seed=0    n=256   mean=78967     p99=262144      [-1.3%]
  ...
  spread across runs: 75604 - 83496  (10.4% of the low end)

POOLED over 10 runs, n=2560 requests:
  mean = 80474
  p50  = 68936      p95 = 183784      p99 = 262144      max = 262144
  vs target mean 80000: +0.6%
  vs context window 262144: p99 reaches 100.0% of it, 31 requests at the window
```

**Report the pooled block, not the target.** It is what was actually served. Measured
results for the two rows:

| row | per-run spread | pooled mean | pooled p99 |
|---|---|---|---|
...
BENCHMARK_SCRIPT=avg_80k_ten   sbatch run_xPyD_models.slurm
BENCHMARK_SCRIPT=avg_200k_ten  sbatch run_xPyD_models.slurm
```
...
**`--max-model-len` is the prerequisite for the long rows.** GLM-5.2 declares
`max_position_embeddings=1048576` and `models.yaml` sets no `--max-model-len`, so it inherits
the full window — **already satisfied, no server change needed**. If anyone later adds a
smaller override, every request above it becomes a 400 with the silent signature above.
...
**KV arithmetic, for sizing.** `(512 + 64) B × 78 layers = 43.88 KiB/token`. So 262,144
tokens is **10.97 GiB** for a single request and 1,048,576 tokens is **43.88 GiB**. That is
why the 1M row runs at half the concurrency of the 256K row — the KV pool sets the ceiling,
not the statistics. The larger sampling error on that row is a consequence of that hardware
limit, which is exactly why it gets reported rather than hidden.
