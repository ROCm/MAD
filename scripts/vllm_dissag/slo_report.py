...
Design notes, because each of these was a real trap:
  ...
  * The customer's throughput targets are PER RANK ("prefill: 34000tokens/s
    ...
    result by exactly the DP degree. --dp-ranks defaults to 8, which is EP8 (1P/1D);
    **an EP16 (2P/2D) run has 16 decode ranks and needs --dp-ranks 16, or every
    per-rank figure below is overstated 2x.**

  * The throughput targets themselves are the customer's MI355X numbers. They are
    printed for reference and are NOT gated -- only TTFT and TPOT are, because those
    are latency SLOs and platform-neutral, whereas comparing MI308X throughput to an
    MI355X target measures the hardware, not the configuration.
...
def main():
    ...
    # Customer's MI355X asks; reported, not gated. See module docstring.
    ap.add_argument("--target-prefill", type=float, default=34000.0)
    ap.add_argument("--target-decode", type=float, default=670.0)
    # 8 = EP8 (1P/1D). Pass 16 for EP16 (2P/2D) or per-rank figures double.
    ...
    print("NOTE throughput targets are the customer's MI355X asks -- reported for")
    print("     reference, NOT gated. Only TTFT and TPOT decide the verdict.")
