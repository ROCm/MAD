...
Two consequences, both deliberate:
  ...
  * We therefore REPORT the deviation. Every run writes <out>.meta.json carrying the
    ...
    1/sqrt(n_total): 10 seeds takes 4.0% -> 1.3% and 9.5% -> 3.0%. That is what
    benchmark_avg_80K_ten.sh / benchmark_avg_200K_ten.sh do.
...
elem/tok/layer, FP8, 78 layers = 43.88 KiB/token. A single 262,144-token request is
therefore 10.97 GiB of KV on its own. This script prints the mean and p99 KV cost per
request so the concurrency you choose is a decision, not an OOM you discover later.
...
# GLM-5.2 MLA KV, bytes/token. See module docstring.
KV_BYTES_PER_TOKEN = (512 + 64) * 1 * 78  # 576 elem * 1 byte (fp8) * 78 layers
...
def kv_report(stats, concurrency_list):
    ...
    if concurrency_list:
        ...
        print("  (one 8x MI355X node has ~930 GiB of KV pool at FP8/util 0.80)")
