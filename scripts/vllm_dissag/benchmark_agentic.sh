...
# inferencex-agentx-mvp scenario against the vLLM router/proxy (:30000) and
# writes aiperf artifacts + an aggregate JSON + plots. Selected by the launcher
# via
...
: "${AGENTIC_PORT:=${BENCHMARK_PORT:-30000}}"              # vLLM router port (BENCHMARK_PORT at runtime)
: "${AGENTIC_CONC:=16}"
: "${DURATION:=120}"
