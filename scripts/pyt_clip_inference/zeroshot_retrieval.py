...
def evaluate(model, dataloader, tokenizer,  device, amp=True, recall_k_list=[5], warmup=False):
    """
    ...
    if not warmup:
        evaluate(model, dataloader, tokenizer,  device, amp, recall_k_list, warmup=True)
    ...
    for batch_images, batch_texts, inds in tqdm(dataloader) if not warmup else dataloader:
        ...
        total_processing_time += this_time 
    ...
    avg_latency = total_processing_time / dataloader_len

    print(f'dataloader_len={dataloader_len}, total_processing_time={total_processing_time}, avg_latency={avg_latency}')
