...
#
# -v4: NO runtime patchers. All K3 KDA/HMA MoRIIO connector support (~69 KDA/GDN
# recurrent+conv state transfer, not just MLA KV) is FOLDED INTO the image's vLLM
# source (v4-disagg @a14e20ae). The image is the contract. See PATCHER_AUDIT.md.
# Per-role: prefill = mori_high_throughput + eager (CG NONE); decode = mori_low_latency
# + cudagraph FULL (needs new MoRI 624002c8; fall back DECODE_CG=PIECEWISE/NONE if it faults).
set -euo pipefail

IMAGE="${IMAGE:-kimik3-wideep-disagg:v4}"
...
if [ "${THOR2_BNXT_FIX}" = "1" ]; then
  ...
  # -v4 / new MoRI 624002c8: expose the FULL host RDMA stack the new MoRI needs for QP
  # setup (librdmacm + netlink + driver config + sysfs). Without these the new MoRI's
  # RDMA-IO QP handshake fails with "transport retry counter exceeded" / responder QP in
  # RESET on Thor2 bnxt, even though the NIC enumerates. Set from clustersphere
  # (cluster_rdma_env_recommender) + MoRI deploy-mori skill. configfs/debugfs are REQUIRED
  # for bnxt DCQCN (mori setup reads/writes congestion control through them).
  # read-only userspace libs the new MoRI's RDMA-CM/netlink QP setup needs. Do NOT mount
  # /etc/libibverbs.d (forces a driver reload that deadlocks the MoRI-EP internode_v1
  # init) nor sysfs (container's own /sys already exposes live QP/port state).
  # Gate the extra RDMA-CM/netlink libs behind THOR2_RDMA_EXTRA (default 1); set 0 to
  # revert to the minimal 2-mount v3 behavior if EP init hangs.
  if [ "${THOR2_RDMA_EXTRA:-1}" = "1" ]; then
    for _m in \
      /usr/lib/x86_64-linux-gnu/librdmacm.so.1 \
      /usr/lib/x86_64-linux-gnu/libnl-3.so.200 \
      /usr/lib/x86_64-linux-gnu/libnl-route-3.so.200 ; do
      [ -e "$_m" ] && BNXT_MOUNTS="$BNXT_MOUNTS -v ${_m}:${_m}:ro"
    done
  fi
else
  BNXT_MOUNTS=""
fi
# MI300X + CX-7 (mlx5, RoCE): expose the host mlx5 RDMA stack for MoRI QP setup, the
# analog of the Thor2 bnxt block. ABI-version mismatch: the image's libibverbs looks for
# the provider at libmlx5-rdmav59.so, but the host driver is libmlx5-rdmav57.so — so mount
# the host v57 AT the image's v59 path (same shim pattern as the bnxt v34->v59 fix).
# Also expose librdmacm + libnl for MoRI's RDMA-CM/netlink QP handshake.
# Enable with FABRIC=mlx5 (and THOR2_BNXT_FIX=0 so BNXT_MOUNTS starts empty).
# ABI FIX (host MLNX OFED 2507 vs image generic rdma-core): the host kernel driver is
# libmlx5-rdmav57 (libibverbs.so.1.14.58.0 / libmlx5.so.1.25.58.0); the image ships only
# rdmav34 + rdmav59 (libibverbs.so.1.16.62.0 / libmlx5.so.1.26.62.0). libibverbs loads the
# provider matching the HOST kernel ABI -> the image has no v57 provider -> RDMA init fails
# ("no provider"/QP setup error), the mlx5 analog of the Thor2 v34-vs-v59 bnxt break. FIX =
# mount the host verbs stack onto the image's RESOLVED sonames (mount the host provider AT
# the image's v59 path, since that's what the image's libibverbs will dlopen). Overridable.
MLX5_PROV_HOST="${MLX5_PROV_HOST:-/usr/lib/x86_64-linux-gnu/libibverbs/libmlx5-rdmav57.so}"
MLX5_PROV_IMG="${MLX5_PROV_IMG:-/usr/lib/x86_64-linux-gnu/libibverbs/libmlx5-rdmav59.so}"
MLX5_LIB_HOST="${MLX5_LIB_HOST:-/usr/lib/x86_64-linux-gnu/libmlx5.so.1.25.58.0}"
MLX5_LIB_IMG="${MLX5_LIB_IMG:-/usr/lib/x86_64-linux-gnu/libmlx5.so.1.26.62.0}"
IBV_LIB_HOST="${IBV_LIB_HOST:-/usr/lib/x86_64-linux-gnu/libibverbs.so.1.14.58.0}"
IBV_LIB_IMG="${IBV_LIB_IMG:-/usr/lib/x86_64-linux-gnu/libibverbs.so.1.16.62.0}"
if [ "${FABRIC:-}" = "mlx5" ]; then
  # host verbs stack onto image sonames (provider first, then the two libs)
  [ -e "$MLX5_PROV_HOST" ] && BNXT_MOUNTS="$BNXT_MOUNTS -v ${MLX5_PROV_HOST}:${MLX5_PROV_IMG}:ro"
  [ -e "$MLX5_LIB_HOST" ]  && BNXT_MOUNTS="$BNXT_MOUNTS -v ${MLX5_LIB_HOST}:${MLX5_LIB_IMG}:ro"
  [ -e "$IBV_LIB_HOST" ]   && BNXT_MOUNTS="$BNXT_MOUNTS -v ${IBV_LIB_HOST}:${IBV_LIB_IMG}:ro"
  # librdmacm + libnl for MoRI's RDMA-CM / netlink QP handshake (host copies)
  for _m in \
    /usr/lib/x86_64-linux-gnu/librdmacm.so.1 \
    /usr/lib/x86_64-linux-gnu/libnl-3.so.200 \
    /usr/lib/x86_64-linux-gnu/libnl-route-3.so.200 ; do
...
case "$ROLE" in
  ...
  decode_master)  DP_ADDR=$DMASTER; KV_ROLE=kv_consumer; BACKEND=mori_low_latency;  CG=FULL_AND_PIECEWISE; HEADLESS=""; START="" ;;
  decode_worker)  DP_ADDR=$DMASTER; KV_ROLE=kv_consumer; BACKEND=mori_low_latency;  CG=FULL_AND_PIECEWISE; HEADLESS="--headless"; START="--data-parallel-start-rank ${DP_LOCAL}" ;;
...
docker run -d --name "$CONTAINER" \
  ...
  -e AITER_SITUV2_A8W4=1 -e VLLM_ROCM_USE_AITER_MOE_SITUV2_A8W4=1 \
  -e VLLM_ROCM_USE_AITER=1 -e VLLM_ROCM_USE_AITER_MOE=1 \
  -e VLLM_ROCM_USE_AITER_MHA=${VLLM_ROCM_USE_AITER_MHA:-1} \
  ...
  -e TORCH_NCCL_DUMP_ON_TIMEOUT=0 -e TORCH_NCCL_BLOCKING_WAIT=0 -e TORCH_NCCL_ASYNC_ERROR_HANDLING=1 \
  -e NCCL_IB_TIMEOUT=22 -e VLLM_SPARSE_INDEXER_MAX_LOGITS_MB=64 \
  ...
  -e MORI_GPU_ARCHS=gfx942 -e MORI_IB_GID_INDEX="${MORI_IB_GID_INDEX:-${IB_GID_INDEX}}" -e MORI_IB_ENABLE_RELAXED_ORDERING=1 \
  -e MORI_NUM_QP_PER_PE=8 -e MORI_SHMEM_HEAP_SIZE=17179869184 \
  -e MORI_RDMA_SL="${MORI_RDMA_SL:-0}" -e MORI_IO_SL="${MORI_IO_SL:-1}" \
  ${MORI_RDMA_TC:+-e MORI_RDMA_TC=${MORI_RDMA_TC}} \
  ${MORI_IO_TC:+-e MORI_IO_TC=${MORI_IO_TC}} \
  ${MORI_IB_TIMEOUT:+-e MORI_IB_TIMEOUT=${MORI_IB_TIMEOUT}} \
  ...
  "$IMAGE" -c "
    ...
    # -v4: flydsl 0.2.4 is baked into the image (Dockerfile step 5b re-pins it after the
    # proven_k3 graft, verified in /app/versions.txt). No runtime bump needed.
    echo \"[disagg] flydsl \$(python3 -c 'import importlib.metadata as m; print(m.version(\"flydsl\"))' 2>/dev/null || echo missing)\"
    # -v4: NO runtime patchers. Every K3 fix (KDA/HMA MoRIIO routing, mamba N-1,
    # group routing, pod-hosts, chunk-gate, port-offset, write-fence) is FOLDED INTO
    # the image's vLLM source (branch v4-disagg @a14e20ae). The a8w4/int4/SiTU cluster
    # is native in upstream d626108b. See PATCHER_AUDIT.md: all 34 former patchers are
    # folded / upstream-superseded / dead. The image is the contract — to change a fix,
    # move VLLM_REF and rebuild, do not re-add runtime patchers.
    # (Lone deferred item: the KVBlockZeroer OOB guard; graft from deferred_patches/
    #  ONLY if a GPU memory access fault appears at high ctx -- see V4_LAUNCH_CONFIG.md.)
    QUANTARG=()
    ...
    # Long-ctx compile (>=320K max-model-len) can exceed vLLM's hardcoded 5-min
    ...
    vllm serve /model --served-model-name kimi-k3 --tensor-parallel-size ${TP_SIZE} \
      ...
      ${THINKING_DEFAULT:+--default-chat-template-kwargs '{\"thinking\":${THINKING_DEFAULT}}'} \
      --no-enable-prefix-caching --kv-cache-dtype ${KV_CACHE_DTYPE:-fp8} --block-size ${BLOCK_SIZE:-16} \
      --kv-cache-memory-bytes ${KV_CACHE_MEMORY_BYTES} \
      --max-model-len ${MAX_MODEL_LEN} --max-num-seqs ${MAX_NUM_SEQS:-8} --max-num-batched-tokens ${MAX_NUM_BATCHED_TOKENS} --gpu-memory-utilization ${GPU_UTIL} \
