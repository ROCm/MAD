#!/bin/bash
# Kimi-K3 (MXFP4) on MI300X / gfx942 -- Wide expert-parallel via true MoRI-EP.
# PP2xTP8 across 2 nodes for weight fit (~102 GB/GPU) PLUS --enable-expert-parallel
# with --all2all-backend mori_low_latency (MoRI-EP all2all kernels) + MoRI RDMA env.
# AITER_SITUV2_A8W4=1 selects the a8w4 (fp8-act x int4-wt) SiTU MoE path. Colocated
# (single instance; no prefill/decode disaggregation).
#
...
docker run -d --name "$CONTAINER" \
  ...
  -e MORI_GPU_ARCHS=gfx942 -e MORI_IB_GID_INDEX=3 -e MORI_IB_ENABLE_RELAXED_ORDERING=1 -e MORI_NUM_QP_PER_PE=8 \
  ...
  "$IMAGE" -c "
    vllm serve /model --served-model-name kimi-k3 \
      ...
      --enable-expert-parallel --all2all-backend mori_low_latency \
      ...
      --gpu-memory-utilization ${GPU_UTIL} ${SERVE_EXTRA} 2>&1 | tee /logs/vllm_moriep_${ROLE}.log
  "
echo "[pp2tp8] $ROLE started. log: $LOGHOST/vllm_moriep_${ROLE}.log"
