...
export LD_LIBRARY_PATH=/usr/local/lib/:/opt/rocm/lib:$LD_LIBRARY_PATH' > $OUTPUT_DIR/maxtext_env_7b.sh
...
$docker run --rm --privileged --network host --device /dev/dri --device /dev/kfd \
  ...
  -w /workspace/maxtext $IMAGE /bin/bash -c "
	...
	cp $OUTPUT_DIR/llama2_7b_gpu.yml configs/llama2_7b_gpu.yml
        ...
        python -m maxtext.trainers.pre_train.train configs/llama2_7b_gpu.yml base_output_directory=output 2>&1 |& tee -a llama2_7b.real.log
    "
