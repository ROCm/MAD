...
#SBATCH --job-name=training_llama2_7B
#SBATCH --output=logs/multinode-job-llama2-7b.%j.out
#SBATCH --time=3:00:00
#SBATCH --nodes=2
...
srun --nodes=$SLURM_JOB_NUM_NODES --ntasks=$SLURM_JOB_NUM_NODES \
    ...
    /bin/bash -c "
        set -e
        echo \"Running Llama-2-7b\"
        echo '${IMAGE}'
        echo \"Coordinator IP: \$JAX_COORDINATOR_IP\"
        cp '${OUTPUT_DIR}'/install_packages.sh .
        cp '${OUTPUT_DIR}'/maxtext_env_7b.sh .
        mkdir -p configs
        cp '${OUTPUT_DIR}'/llama2_7b_gpu.yml configs/llama2_7b_gpu.yml
        source install_packages.sh
        source maxtext_env_7b.sh
        python -m maxtext.trainers.pre_train.train configs/llama2_7b_gpu.yml 2>&1 |& tee -a llama2_7b.synthetic.log
    "'