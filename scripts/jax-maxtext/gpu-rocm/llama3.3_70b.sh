...
$docker run --rm --privileged --network host --device /dev/dri --device /dev/kfd \
  ...
  -w /workspace/maxtext $IMAGE /bin/bash -c "
	...
	mkdir -p configs
	cp $OUTPUT_DIR/llama3.3_70b_gpu.yml configs/llama3.3_70b_gpu.yml
        source maxtext_env_70b.sh	
        python -m maxtext.trainers.pre_train.train configs/llama3.3_70b_gpu.yml base_output_directory=output 2>&1 |& tee -a llama3_70b.real.log
    "
