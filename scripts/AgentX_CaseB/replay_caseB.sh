#!/bin/bash
# =============================================================================
# AgentX Case-B conformance trace — generic replay driver.
#
# Replays the frozen Case-B trace against ANY OpenAI-compatible endpoint and
...
if [ ! -d "$CORP" ] || [ -z "$(ls "$CORP"/*.json 2>/dev/null)" ]; then
  if [ -f "$HERE/caseB_conformance_corpus.tar.gz" ] && [ "$CORP" = "$HERE/corpus" ]; then
    ...
    tar xzf "$HERE/caseB_conformance_corpus.tar.gz" -C "$HERE"
  else
    echo "[replay] regenerating trace (seed=$SEED, n=$NSESS) -> $CORP"
    python3 "$HERE/gen_caseB_conformance.py" "$CORP" "$NSESS" "$SEED"
  fi
fi
