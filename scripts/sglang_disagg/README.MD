...
- `DP_MODE=1` enables expert parallelism (EP) with DP attention and currently requires `RUN_MORI=1` (MoRI IO). The all-to-all backend comes from the model's `dp_flags`, not from the mode: most entries select `--moe-a2a-backend mori`, `Kimi-K2-Instruct-DeepEP-AB` selects `--moe-a2a-backend deepep --deepep-mode normal`.
...
- All three Kimi entries are registered in `models.json` as `sglang-disagg-kimi-k2-instruct-overlay`, `...-mori-ab` and `...-deepep-ab`, so they are selectable from the madengine catalog. They use `run.sh` directly, the path they were measured on; `MODEL_PATH` still comes from the deployment.
- `Kimi-K2-Instruct-MoRI-AB` and `Kimi-K2-Instruct-DeepEP-AB` are configuration aliases of `Kimi-K2-Instruct`, differing only in `dp_flags`, and resolve to its weights directory. They vary one flag with everything else held equal; neither is a tuned configuration, so a throughput quoted from either is a comparison point, not a product number.
...
|---|---|---|
| `PROFILE_ENABLE=1` | Adds `--disable-custom-all-reduce` and `--disable-cuda-graph` to **both** roles, so collectives reach a profiler instead of hiding inside a replayed graph or sglang's own all-reduce | Throughput. Read it from a second run without this |
