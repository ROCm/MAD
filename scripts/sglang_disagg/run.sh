...
if ! _weights_complete; then
    ...
    if [[ -z "${_MODEL_REPO}" ]]; then
        case "${MODEL_NAME}" in
            ...
            # share its weights, so they stage from its repository. Reached when MODEL_PATH is
            # set but holds no complete weights and MODEL_REPO is unset -- an empty MODEL_PATH
            # has already exited above. Their catalog entries set MODEL_REPO; this covers the
