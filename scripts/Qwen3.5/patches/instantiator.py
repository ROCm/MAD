...
class _StringLoader(importlib.abc.SourceLoader):
    ...
    def get_filename(self, fullname):
        # PATCH: return a path WITH a .py suffix. Upstream returns bare `fullname`
        # (no extension), which makes importlib.cache_from_source() do string slicing
        # that throws "IndexError: string index out of range" on this CPython/torch
        # combo. A proper .py-suffixed path makes cache path computation well-formed.
        return fullname + ".py"
